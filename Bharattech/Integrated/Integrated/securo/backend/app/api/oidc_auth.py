...
def _backend_url(url: str) -> str:
    ...
    internal = (getattr(settings, "oidc_internal_url", "") or "").rstrip("/")
    public = (getattr(settings, "oidc_public_url", "") or "").rstrip("/")
    ...
    for prefix in (
        ...
        if url.startswith(prefix):
            return url.replace(prefix, internal, 1)
    return url
...
def _browser_url(url: str) -> str:
    ...
    for prefix in (
        ...
        if url.startswith(prefix):
            return url.replace(prefix, public, 1)
    return url
...
async def _discover() -> dict[str, Any]:
    ...
    url = _backend_url(settings.oidc_discovery_url)
...
async def oidc_login():
    ...
    raw_auth_endpoint = discovery.get("authorization_endpoint")
    if not raw_auth_endpoint:
        raise HTTPException(status_code=500, detail="OIDC discovery is missing authorization_endpoint")
    authorization_endpoint = _browser_url(raw_auth_endpoint)
...
async def _exchange_code(discovery: dict[str, Any], code: str, code_verifier: str) -> dict[str, Any]:
    ...
    headers = {**_host_header(), "Accept": "application/json"}
    async with httpx.AsyncClient(timeout=10) as client:
...
async def _fetch_userinfo(discovery: dict[str, Any], access_token: str) -> dict[str, Any]:
    raw_userinfo_endpoint = discovery.get("userinfo_endpoint")
    if not raw_userinfo_endpoint or not access_token:
        return {}
    userinfo_endpoint = _backend_url(raw_userinfo_endpoint)
    headers = {"Authorization": f"Bearer {access_token}", **_host_header()}
    try:
        async with httpx.AsyncClient(timeout=10) as client:
...
async def _decode_id_token(
    ...
    settings = get_settings()
    raw_jwks_uri = discovery.get("jwks_uri")
    issuer = discovery.get("issuer")
    if not raw_jwks_uri or not issuer:
        raise HTTPException(status_code=500, detail="OIDC discovery is missing jwks_uri or issuer")
    jwks_uri = _backend_url(raw_jwks_uri)
    ...
    try:
        ...
        expected_issuers = {issuer, _browser_url(issuer), _backend_url(issuer)}
        valid_issuer = token_issuer if token_issuer in expected_issuers else issuer

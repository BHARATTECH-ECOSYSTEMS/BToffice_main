...
export default function InternDashboard() {
  ...
  const internEmail = localStorage.getItem("internEmail") || null;

  const { user: authUser, logout, hasRole } = useAuth();

  useEffect(() => {
    if (!authUser || !hasRole("Intern")) {
  ...
  const loadResetLinks = async () => {
    try {
      console.log("🔄 Loading reset links for intern...");
      const links = await getResetLinksByRole("Intern");
      console.log(`✅ Loaded ${links.length} reset links for intern`);
      setResetLinks(links);
    } catch (error) {
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      // Filter tasks assigned to this intern (by email or role)
      const internEmail = localStorage.getItem("internEmail");
      const my = tasksArray.filter(t => {
        ...
        return assignedEmail === internEmail || assignedRole === "Intern";
      });
  ...
  const handleFileUpload = (taskId, file) => {
    ...
    addTaskFile(taskId, file.name, url, internEmail);
    loadTasks();
          ...
          <main className="flex-1 p-3 sm:p-4 md:p-6 lg:p-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6 md:mb-8">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold text-gray-900 leading-tight">Intern Dashboard</h1>
                  <p className="text-[10px] sm:text-xs md:text-sm text-gray-600 mt-0.5 sm:mt-1 leading-tight">View tasks, update status, and upload assignments.</p>
                </div>
              </div>
            ...
            <Card className="mb-4 sm:mb-6">
              <CardHeader className="px-3 sm:px-4 md:px-6 py-3 sm:py-4">
                <CardTitle className="text-base sm:text-lg font-semibold">My Tasks (Interns)</CardTitle>
              </CardHeader>
              ...
              <CardContent className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6">
                {isLoading ? (
                  ...
                  <div className="space-y-2 sm:space-y-3 md:space-y-4">
                    {tasks.map(task => (
                      <div key={task.id} className="p-2.5 sm:p-3 md:p-4 bg-gray-50 rounded-md hover:bg-gray-100 transition flex flex-col sm:flex-row sm:justify-between gap-2.5 sm:gap-3 md:gap-4">

...
const getInitialUser = () => {
  ...
  const roleFromJwt = jwtRoles.find((r) =>
    ["admin", "superadmin", "super-admin", "subadmin"].includes(String(r).toLowerCase())
  );
  ...
  const user = {
    ...defaultStudentUser,
    ...storedUser,
    id: params.get("userId") || jwtPayload?.sub || storedUser?.id || email || defaultStudentUser.id,
    username: name,
    fullName: name,
    name,
    email,
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams(window.location.search);
    if (params.has("token") || params.has("accessToken") || params.has("refresh_token")) {
      params.delete("token");
  ...
  const getToken = async () => {
    ...
    // 2. Local/Session Storage (check if valid and not expired)
    const storedToken =
      localStorage.getItem("token") ||
      localStorage.getItem("accessToken") ||
      sessionStorage.getItem("bharattechLmsToken") ||
    ...
    // 3. Stored token is missing or expired -> Auto-refresh using refresh token

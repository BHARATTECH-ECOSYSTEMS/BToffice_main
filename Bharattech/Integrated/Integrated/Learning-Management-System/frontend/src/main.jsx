...
const keycloakClientId =
  import.meta.env.VITE_KEYCLOAK_CLIENT_ID || "lms-client";
...
if (shouldUseDemoAuth) {
  ...
  if (urlToken || urlUserRaw) {
    ...
    if (urlIdToken) {
      localStorage.setItem("id_token", urlIdToken);
    }
    ...
    if (urlUserRaw) {
      try {
        ...
        if (pu.role) {
          localStorage.setItem("role", pu.role);
        }

        if (pu.username) {
          localStorage.setItem("username", pu.username);
        }

        if (pu.email) {
          localStorage.setItem("userEmail", pu.email);
        }

        if (pu.fullName) {
          localStorage.setItem("userFullName", pu.fullName);
        }

        if (pu.firstName) {
          localStorage.setItem("userFirstName", pu.firstName);
        }

        if (pu.lastName) {
          localStorage.setItem("userLastName", pu.lastName);
        }
      } catch {
  ...
  function bootstrapFromLocalToken(token) {
    try {
      const payload = JSON.parse(
        atob(token.replace(/-/g, "+").replace(/_/g, "/").split(".")[1]),
      );

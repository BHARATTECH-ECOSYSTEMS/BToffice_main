...
/**
 * Run SecuPrompt on a subagent control message by temporarily exposing
 * body.message as body.text — mirroring moderateSubagentControlMessage.
 * If SecuPrompt sanitizes the text, the sanitized version is written back
 * to body.message so the controller receives the cleaned content.
 */
async function secuPromptSubagentControlMessage(req, res, next) {
  const body = (req.body ??= {});
  if (!['steer', 'queue', 'interrupt'].includes(body.action)) {
    next();
    return;
  ...
  const restore = () => {
    if (hadText) {
      ...
      // If SecuPrompt mutated body.text (sanitize mode), reflect it to body.message
      if (typeof body.text === 'string' && body.text !== body.message) {
  ...
  try {
    await secuPromptMiddleware(req, res, (error) => {
      restore();
      next(error);
    });
  } finally {
...
router.post(
  ...
  secuPromptSubagentControlMessage,

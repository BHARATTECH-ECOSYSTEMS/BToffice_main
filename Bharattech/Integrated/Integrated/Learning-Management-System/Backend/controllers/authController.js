...
const sendVerificationEmail = async (email, token) => {
  ...
  if (EMAIL_USER && EMAIL_PASS) {
    ...
    throw new Error(
      "SMTP email is not configured. Please check VITE_AUTH_MODE / EMAIL_USER environment variables.",
    );
  }
  ...
  try {
    ...
    if (!isPasswordHashed) {
      ...
      return res.status(500).json({
        message:
          "Account setup incomplete. Please contact admin to reset your password.",
      });
    }
    ...
    if (!isMatch) {
      ...
      console.log(
        "💡 User exists but password doesn't match. User may need to reset password.",
      );
      return res.status(400).json({
        message:
          "Invalid email or password. If you're a new user, please use the password reset link provided by your admin.",
      });
    }
...
const signup = async (req, res) => {
  try {
    ...
    if (userExists) {
      return res
        .status(409)
        .json({ message: "Username or Email already exists in local DB" });
    }
    ...
    if (process.env.AUTH_MODE === "keycloak") {
      ...
      if (matches.length > 0) {
        return res
          .status(409)
          .json({ message: "Username already exists in Keycloak" });
      }
      ...
      if (emailMatches.length > 0) {
        return res
          .status(409)
          .json({ message: "Email already exists in Keycloak" });
      }
    }
...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (!token) {
      return res.redirect(
        "http://localhost:8080/employees?verified=false&error=Missing+token",
      );
    }
    ...
    if (!verification) {
      return res.redirect(
        "http://localhost:8080/employees?verified=false&error=Invalid+or+expired+verification+token",
      );
    }
    ...
    if (process.env.AUTH_MODE === "keycloak") {
      console.log(
        `📡 Keycloak Sign Up (Verified): Creating user ${resolvedUsername} (${resolvedEmail})`,
      );
      const kcAdmin = new KcAdminClient({
        baseUrl: KEYCLOAK_BASE_URL,
        realmName: KEYCLOAK_ADMIN_REALM,
    ...
    return res.redirect(
      `http://localhost:8080/employees?verified=false&error=${encodeURIComponent(error.message || "Verification failed")}`,
    );
  }
};
...
const acceptInvite = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Username, temporary password, new password, and a chosen username are required",
      });
    }
    ...
    if (
      !checkResult.ok &&
      !/not fully set up/i.test(checkResult.errorDescription || "")
    ) {
      return res
        .status(401)
        .json({ message: "Invalid username or temporary password" });
    }
    ...
    const byUsername = await kcAdmin.users.find({
      realm: KEYCLOAK_REALM,
      username,
      exact: true,
    });
    const byEmail = byUsername.length
      ? []
    ...
    if (
      trimmedNewUsername.toLowerCase() !== (kcUser.username || "").toLowerCase()
    ) {
      const taken = await kcAdmin.users.find({
        realm: KEYCLOAK_REALM,
    ...
    const loginResult = await passwordGrantLogin(
      trimmedNewUsername,
      newPassword,
    );
    ...
    if (!loginResult.ok) {
      console.error(
        "Post-accept-invite login failed:",
        loginResult.errorDescription || loginResult.error,
      );
      return res.status(500).json({
        message:
          "Password updated, but automatic sign-in failed. Please log in again.",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to set new password", error: error.message });
  }
};
...
const keycloakChangePassword = async (req, res) => {
  try {
    ...
    if (!username) {
      return res
        .status(401)
        .json({ message: "Could not identify the current user" });
    }

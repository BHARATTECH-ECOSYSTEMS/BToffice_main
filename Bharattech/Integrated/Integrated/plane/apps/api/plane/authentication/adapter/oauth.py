...
class OauthAdapter(Adapter):
    ...
    def get_user_token(self, data, headers=None):
        try:
            ...
            self.logger.warning(
                f"Error getting user token: {e} | status={getattr(e.response, 'status_code', None)}"
            )
            code = self.authentication_error_code()
            raise AuthenticationException(error_code=AUTHENTICATION_ERROR_CODES[code], error_message=str(code))

    def get_user_response(self, headers=None):
        try:
            req_headers = {"Authorization": f"Bearer {self.token_data.get('access_token')}"}
            if headers:
                req_headers.update(headers)
            response = requests.get(self.get_user_info_url(), headers=req_headers)
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
            # Do not log headers here: they carry the access token
            self.logger.warning(
                f"Error getting user response: {e} | status={getattr(e.response, 'status_code', None)}"
            )
            code = self.authentication_error_code()
            raise AuthenticationException(error_code=AUTHENTICATION_ERROR_CODES[code], error_message=str(code))
